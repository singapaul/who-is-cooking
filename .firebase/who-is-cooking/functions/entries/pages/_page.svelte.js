...
const SortableList = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	return `${
		list?.length
			? `<ul class="flex list-none flex-col items-center p-0">${each(list, (item, index) => {
					return `<li class="${[
						'w-full max-w-md border-2 border-dashed border-transparent p-2 transition-all svelte-16x08xz',
						item.id === isOver ? 'over' : ''
					]
						.join(' ')
						.trim()}"${add_attribute('data-index', index, 0)}${add_attribute('data-id', item.id, 0)} draggable="true">${slots.default ? slots.default({ item, index }) : ``} </li>`;
				})}</ul>`
			: `<p class="my-12 text-center text-lg font-bold" data-svelte-h="svelte-1ideuqf">No items</p>`
	}`;
});
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	return `${validate_component(AuthCheck, 'AuthCheck').$$render(
		...
		{
			default: () => {
				return `<main class="m-auto mx-auto flex max-w-xl flex-col items-center gap-6 py-9"><h1 class="text-2xl font-bold" data-svelte-h="svelte-1y66r94">Cooking Rota</h1> ${validate_component(
					SortableList,
					'SortableList'
				).$$render(
					$$result,
					...
					{
						default: ({ item, index }) => {
							return `<div class="group relative"> ${validate_component(
								ChefLink,
								'ChefLink'
							).$$render(
								$$result,
