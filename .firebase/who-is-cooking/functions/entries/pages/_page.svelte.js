...
const ChefLink = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return ` <a${add_attribute("class", `not-prose stack flex w-full min-w-80 max-w-md items-center justify-center rounded-lg border bg-base-300 p-4 text-center no-underline ${checkedStatus ? "border-green-500" : "border-white"}`, 0)}><p class="text-2xl font-bold">${escape(name)}</p> <div class="form-control"><label class="label cursor-pointer"><span class="label-text" data-svelte-h="svelte-1hpt7xr">Cooked/ Unavailable</span> <input type="checkbox" ${checkedStatus ? "checked" : ""} class="checkbox-primary checkbox"> ${sparkles ? `${validate_component(Confetti, "Confetti").$$render($$result, {}, {}, {})}` : ``}</label></div></a>`;
});
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${validate_component(AuthCheck, "AuthCheck").$$render($$result, {}, {}, {
    default: () => {
      ...
      })}  <div class="flex flex-col gap-4 w-full"><button class="btn btn-accent w-full" data-svelte-h="svelte-1spidb8">Re-order rota</button> <button class="btn btn-warning w-full" data-svelte-h="svelte-11nvtcx">Hard reset rota</button></div></main>`;
    }
  })}`;
});
export {
