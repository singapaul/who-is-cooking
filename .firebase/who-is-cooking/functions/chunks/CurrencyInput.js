...
const CurrencyInput = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.isNegativeAllowed === void 0 && $$bindings.isNegativeAllowed && isNegativeAllowed !== void 0)
  ...
  return `<div${add_attribute("class", inputClasses?.wrapper ?? DEFAULT_CLASS_WRAPPER, 0)}><input class="${escape(null_to_empty(inputClasses?.unformatted ?? DEFAULT_CLASS_UNFORMATTED), true) + " svelte-n0twf6"}" type="hidden"${add_attribute("name", name, 0)} ${disabled ? "disabled" : ""}${add_attribute("value", value, 0)}> <input class="${"" + escape(inputClasses?.formatted ?? DEFAULT_CLASS_FORMATTED, true) + " " + escape(
    ...
    true
  ) + " svelte-n0twf6"}" type="text"${add_attribute("inputmode", fractionDigits > 0 ? "decimal" : "numeric", 0)}${add_attribute("name", `formatted-${name}`, 0)} ${required && !isZero ? "required" : ""}${add_attribute("placeholder", handlePlaceholder(placeholder), 0)}${add_attribute("autocomplete", autocomplete, 0)} ${disabled ? "disabled" : ""}${add_attribute("this", inputElement, 0)}${add_attribute("value", formattedValue, 0)}> </div>`;
});
export {
  CurrencyInput as C,
  FormInput as F,
