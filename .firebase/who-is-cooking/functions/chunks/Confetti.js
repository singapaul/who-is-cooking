...
const Confetti = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	if (
		$$props.destroyOnComplete === void 0 &&
		$$bindings.destroyOnComplete &&
		destroyOnComplete !== void 0
	)
	...
	return `${`<div class="${[
		...
		.join(' ')
		.trim()}">${each({ length: amount }, (_) => {
		return `<div class="confetti svelte-io58ff" style="${
			'--fall-distance: ' +
			escape(fallDistance, true) +
			'; --size: ' +
			escape(size, true) +
			'px; --color: ' +
			escape(getColor(), true) +
			'; --skew: ' +
			escape(randomBetween(-45, 45), true) +
			'deg,' +
			escape(randomBetween(-45, 45), true) +
			'deg; --rotation-xyz: ' +
			escape(randomBetween(-10, 10), true) +
			', ' +
			escape(randomBetween(-10, 10), true) +
			', ' +
			escape(randomBetween(-10, 10), true) +
			'; --rotation-deg: ' +
			escape(randomBetween(0, 360), true) +
			'deg; --translate-y-multiplier: ' +
			escape(randomBetween(y[0], y[1]), true) +
			'; --translate-x-multiplier: ' +
			escape(randomBetween(x[0], x[1]), true) +
			'; --scale: ' +
			escape(0.1 * randomBetween(2, 10), true) +
			'; --transition-duration: ' +
			escape(infinite ? `calc(${duration}ms * var(--scale))` : `${duration}ms`, true) +
			'; --transition-delay: ' +
			escape(randomBetween(delay[0], delay[1]), true) +
			'ms; --transition-iteration-count: ' +
			escape(infinite ? 'infinite' : iterationCount, true) +
			'; --x-spread: ' +
			escape(1 - xSpread, true)
		}"></div>`;
	})}</div>`}`;
});
